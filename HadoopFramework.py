...
class readcontrolfile():
    ...
    def __init__(self,parmfile="/Users/vigneshwaransundaresan/MetlifeHadoopFramework/Parmfiles/test_parm.parm"):
        ...
        logging = logger(logfile,self.jobname,'HCF_Quality_Checks').extract()

        logging.info("Quality Checks started for Job : {}".format(self.jobname))

        if empty_file_check_indicator.upper() == 'Y':

            empty_check = performQchecks.emptyfilecheck(self,logfile,source_file_path,source_data_file_name)

        if identity_check_indicator.upper() == 'Y':

            identity_check = performQchecks.identitycheck(self,logfile,source_file_path,source_data_file_name)

        if qc_check_ind.upper() == 'Y':

            qc_check = performQchecks.qccheck(self,logfile,source_file_path,source_qc_file)
